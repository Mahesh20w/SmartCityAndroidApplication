...
if (isset($_POST['Login'])) {
   /* if (isset($_REQUEST['redirurl']))
        ...
        $url = "index.php";
*/
    $sql = mysqli_query($conn, "select * from admin where admin_id='" . $_POST["Username"] . "' and password='" . $_POST["Password"] . "'");

    if (mysqli_num_rows($sql) > 0) {
        while ($results = mysqli_fetch_assoc($sql)) {
			$_SESSION['eid']=$results['admin_id'];
			...
			<script> alert('Login succesfully')
		               window.location.href='main_home_admin.php';
		</script>
		?>
		<?php
        }
